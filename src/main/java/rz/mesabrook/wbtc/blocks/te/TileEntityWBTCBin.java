...
public class TileEntityWBTCBin extends TileEntityLockableLoot implements ITickable
{
	private NonNullList<ItemStack> binContents = NonNullList.<ItemStack>withSize(72, ItemStack.EMPTY);
	...
	{
		for(ItemStack stack : this.binContents)
		{
	...
	public void readFromNBT(NBTTagCompound compound)
	{
		...
		this.binContents = NonNullList.<ItemStack>withSize(this.getSizeInventory(), ItemStack.EMPTY);
		
		if(!this.checkLootAndRead(compound)) ItemStackHelper.loadAllItems(compound, binContents);
	...
	public NBTTagCompound writeToNBT(NBTTagCompound compound)
	{
		...
		if(!this.checkLootAndWrite(compound)) ItemStackHelper.saveAllItems(compound, binContents);
	...
	{
		return new ContainerWBTCBin(playerInventory, this, playerIn);
	}
	
	@Override
	public String getGuiID() 
	{
		return "wbtc:trash_bin";
	}
	...
	public void update()
	{
		if(!this.world.isRemote && this.numPlayersUsing != 0 && (this.ticksSinceSync + pos.getX() + pos.getY() + pos.getZ()) % 200 == 0)
		{
            ...
            {
                if (entityplayer.openContainer instanceof ContainerWBTCBin)
                {
                    if (((ContainerWBTCBin)entityplayer.openContainer).getChestInventory() == this)
                    {
